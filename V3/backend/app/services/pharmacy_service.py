...
def create_medicine(db: Session, hospital_id: uuid.UUID, data: dict, user_id: uuid.UUID) -> Medicine:
    med = Medicine(hospital_id=hospital_id, created_by=user_id, **data)
    db.add(med)
    db.commit()
    db.refresh(med)
    return med
...
def create_batch(db: Session, data: dict) -> MedicineBatch:
    ...
    for fk in ("medicine_id", "supplier_id", "purchase_order_id"):
        if data.get(fk):
...
def create_supplier(db: Session, hospital_id: uuid.UUID, data: dict) -> Supplier:
    sup = Supplier(hospital_id=hospital_id, **data)
    db.add(sup)
    db.commit()
    db.refresh(sup)
    return sup
        ...
        if item.batch_number:
            ...
            if existing:
                ...
                batch = MedicineBatch(
                    ...
                    selling_price=item.unit_price,  # default; can be updated
...
def create_sale(
    ...
    for item_data in items_data:
        ...
        if batch_id:
            ...
            if batch and batch.quantity >= qty:
                batch.quantity -= qty
            else:
        ...
        # Resolve supplier name from batch
        supplier_name = None
        ...
        si = PharmacySaleItem(
            ...
            mrp=batch.mrp if batch else None,
    ...
    if search:
        ...
        query = query.filter(
            or_(
                ...
                PharmacySale.patient_name.ilike(s),
            )
        )
    if date_from:
...
def get_pharmacy_dashboard(db: Session, hospital_id: uuid.UUID) -> dict:
    ...
    today_sales = db.query(
        ...
        PharmacySale.hospital_id == hospital_id,
        func.date(PharmacySale.sale_date) == today,
    ).first()
