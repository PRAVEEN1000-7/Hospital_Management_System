...
def _patient_read_or_write_with_pharmacist(level: str):
    """Narrow carve-out letting a pharmacist, optical_staff, or lab_technician
    user search for/view an existing patient, or register a new one, from
    their own walk-in flows (PrescriptionBuilder.tsx's patient search +
    ...
    lab_technician) — without granting the full "general.patients" edit
    right, which also gates updating and soft-deleting ANY patient's
    demographic record (patient_update_role_guard / patient_delete_role_guard
    below, deliberately left untouched by this carve-out). Only the three
    endpoints these roles actually need here (list/search, get one, create)
    use this guard instead.

    None of these three roles has any entry at all in the "general.patients"
    ...
    NewLabOrder.tsx/LabReferralForm.tsx's patient search until this was
