...
_PATIENT_SEARCH_CARVEOUT_ROLES = {"pharmacist", "optical_staff", "lab_technician", "cashier"}


def _patient_read_or_write_with_pharmacist(level: str):
    """Narrow carve-out letting a pharmacist, optical_staff, lab_technician, or
    cashier user search for/view an existing patient, or register a new one,
    from their own walk-in flows (PrescriptionBuilder.tsx's patient search +
    "Register as new patient" round trip via Register.tsx for pharmacist;
    NewOpticalPrescription.tsx / NewOpticalSale.tsx's own patient search +
    the same Register.tsx round trip for optical_staff; NewLabOrder.tsx /
    LabReferralForm.tsx's patient search + the same round trip for
    lab_technician; NewRefund.tsx's patient search for cashier) — without
    granting the full "general.patients" edit right, which also gates
    updating and soft-deleting ANY patient's demographic record
    (patient_update_role_guard / patient_delete_role_guard below,
    deliberately left untouched by this carve-out). Only the three
    endpoints these roles actually need here (list/search, get one, create)
    use this guard instead.

    None of these four roles has any entry at all in the "general.patients"
    matrix (see module_roles.py), so every patient search in their modules
    403'd silently — e.g. NewOpticalPrescription.tsx's search effect
    swallows the error into an empty suggestion list, which read as "the
    patient number isn't suggesting anything" rather than an obvious
    permission error. lab_technician had the exact same gap in
    NewLabOrder.tsx/LabReferralForm.tsx's patient search, and cashier the
    same gap in NewRefund.tsx's patient search, until each was added
    ...
    async def _dependency(
        ...
        if check_permission(db, current_user, "general.patients", level):
            return current_user
        logger.warning(
            "RBAC deny (general.patients/pharmacist+optical_staff+lab_technician carve-out) user=%s roles=%s level=%s",
