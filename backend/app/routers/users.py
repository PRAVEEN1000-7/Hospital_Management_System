...
async def create_new_user(
    ...
    try:
        ...
        # Only non-CORE module codes that exist in the modules table are checked.
        # CORE modules (patients, doctors, appointments, auth, hospital_profile) are
        # always available. Permission module strings like 'reports', 'admin', 'prescription'
        # don't correspond to actual module codes and are intentionally skipped.
        # Roles such as doctor/nurse/receptionist rely solely on CORE modules and should
        # always be creatable.
        _role_module_map: dict[str, list[str]] = {
            'pharmacist':        ['pharmacy'],
            'cashier':           ['billing'],
        ...
        if required_optional and "super_admin" not in current_user.roles:
            ...
            if tenant:
                ...
                missing = [m for m in required_optional if m not in enabled_module_codes]
                if missing:
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
