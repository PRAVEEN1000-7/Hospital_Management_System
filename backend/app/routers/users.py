...
async def create_new_user(
    ...
    send_email: bool = Query(True, description="Send credentials via email"),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_super_admin),
):
    ...
    try:
        ...
        if send_email:
            try:
                ...
                logger.warning(f"User created but email failed: {e}")
                # User is created, just log the email failure
...
async def reset_user_password(
    ...
    try:
        ...
        if send_email_flag and email_sent:
            msg += " and sent via email"
        elif send_email_flag and email_error:
...
async def send_password_to_user(
    ...
    try:
        ...
        # Send email — RuntimeError will be caught and returned as 400
        try:
            email_sent = send_password_email(
                to_email=target_user.email,
                username=target_user.username,
            ...
            return {
                "message": f"Password updated but email failed: {str(e)}",
                "email_sent": False,
            }
