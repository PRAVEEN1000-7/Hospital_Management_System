...
def create_hospital(db: Session, hospital_data: HospitalCreate, user_id: int) -> HospitalDetails:
    ...
    # Remove fields not in the database model to avoid unexpected keyword arguments
    data.pop('logo_path', None)
    data.pop('logo_filename', None)
    data.pop('logo_mime_type', None)
    data.pop('logo_size_kb', None)
    
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"Creating hospital with data keys: {list(data.keys())}")
