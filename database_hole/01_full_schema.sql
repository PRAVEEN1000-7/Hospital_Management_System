...
--      shifts, holidays, payroll,
--      allowances, incentives, advances)   ← 10-26_*.sql (attendance module)
...
-- password_reset_tokens — same shape as refresh_tokens (§2.6 above), for
-- the "Forgot Password" flow (routers/auth.py). Source: 24_add_password_
-- reset_tokens.sql. This table was referenced by models/user.py and used
-- live by auth.py from the start, but no prior version of this file (nor
-- any numbered migration) ever actually created it — a genuine gap, not a
-- later addition, which is why it's folded in here next to its sibling
-- token tables rather than filed under a later section.
...
--         20_add_payroll.sql, 21_add_shift_assignment_history.sql,
--         22_add_allowances.sql, 23_add_incentives.sql
--
