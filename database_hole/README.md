...
| `2026-08-16_seed_lab_test_panels_data.sql` | Data for the table above — creates the MHC (Master Health Checkup) and HHC (Health Checkup) packages for every hospital, plus two catalog tests (CBC, standalone TSH) they need that were missing from the original seed. Resolves each package's tests by name per hospital, so it's portable across databases; skips a hospital missing a required test rather than erroring. **Requires `2026-08-14_lab_test_panels.sql` first.** Data only, idempotent (`ON CONFLICT DO NOTHING` / `DO UPDATE`). | ✅ Yes |
