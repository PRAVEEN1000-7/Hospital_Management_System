...
| `2026-08-16_seed_lab_test_panels_data.sql` | Data for the table above — creates the MHC (Master Health Checkup) and HHC (Health Checkup) packages for every hospital. **Rewritten 2026-08-18**: now creates all 9 distinct catalog tests either package needs itself (by code, `ON CONFLICT DO NOTHING`), then resolves each package's members by that same code — no longer assumes any of those 9 already exist from another migration or from manual local setup. (The original version matched by name and assumed names from the author's local dev DB — 4 of the 9 existed on no migration at all, only created locally by hand — would also exist verbatim on production; they didn't, so the packages silently never got created there.) Skips a hospital only if creating its 9 tests itself somehow still didn't cover a package's full set (shouldn't happen — flag it if it does). **Requires `2026-08-14_lab_test_panels.sql` first.** Data only, idempotent. | ✅ Yes |
| `2026-08-17_lab_order_item_billed_name.sql` | Adds `lab_order_items.billed_name` — pairs with a code change where the Lab Test Catalog no longer accepts a price at all (`lab_tests.price` stays ₹0 for every test); billing staff now enter the real amount **and** an optional custom display name per order item, at collection time (`LabCollectPayment.tsx`). `billed_name` feeds the invoice and billing worklist only — the doctor's report/PrescriptionBuilder keep showing the catalog `test_name`, untouched. Also zeroes any pre-existing nonzero `lab_tests.price` rows (existing order items are left as-is). Schema + one-time data cleanup. | ✅ Yes |
| `2026-08-17_enable_lab_module_default.sql` | Enables the `lab` module (`saas_core.tenant_modules`) for every existing tenant, and adds it to every subscription plan's `modules_included` so future hospitals get it too. Root cause of "Laboratory Tests missing for some doctors": the catalog was already fully seeded everywhere, but most tenants had no `tenant_modules` row (or a disabled one) for `lab`, so `is_module_enabled('lab')` was false and PrescriptionBuilder.tsx's whole Laboratory Tests card never rendered. Never force-disables — only flips missing/disabled rows to enabled, matching `tenant_service.py`'s own "only upgrade" rule. Data only, idempotent. | ✅ Yes |
| `2026-08-20_optical_prescription_exam_fields.sql` | Adds 6 columns to `optical_prescriptions` — `right_vision`/`left_vision` (presenting/unaided vision, distinct from the existing `right_va`/`left_va` which is vision WITH this prescription's correction), `right_iop`/`left_iop` (tension, via Schiotz tonometer), `right_nld`/`left_nld` (nasolacrimal duct patency) — for the nurse's pre-consultation Optical entry screen. All free-text (VARCHAR), same reasoning as the existing VA columns. Schema only. | ✅ Yes |
| `2026-08-20_revert_doctor_no_show_status.sql` | Data cleanup paired with removing the "No Show" action from the doctor's own screens (`DoctorAppointments.tsx`, `WalkInQueue.tsx` Scheduled tab) — reverts any appointment currently `no-show` back to its status immediately before that transition, recovered from `appointment_status_log`, but **only** when the transition was performed by a user holding the `doctor` role (reception/admin's own separate no-show marking is untouched). Also writes a new status-log row for the revert itself so the audit trail stays continuous. Data only, idempotent. | ✅ Yes |
| `2026-08-21_fix_legacy_workforce_schema_drift.sql` | Fixes `holidays`/`attendance_records`/`payroll_runs` on any database where these tables already existed with an older, pre-Section-8 shape (one-row-per-date holidays with `name`/`type`; an `employee_id`/`is_verified`/`verified_by` attendance workflow; `period_month`/`period_year` payroll runs) — because `workforce_attendance_module_combined.sql` (and `01_full_schema.sql` §8) use `CREATE TABLE IF NOT EXISTS`, they silently left those older tables untouched instead of upgrading them, causing `column holidays.year does not exist` / `column attendance_records.user_id does not exist` 500s on every Holiday Calendar / Attendance Report / Leave / Payroll request. **Preserves existing data**: old per-date holiday rows are consolidated into the new one-row-per-(hospital,year,month) shape (split into `holiday_days`/`festival_days` by the old `type` column); `attendance_records.employee_id` is renamed to `user_id`; old `'holiday'`-status attendance rows are dropped (redundant with the now-migrated `holidays` table — the current design computes Week-Off/Holiday on read, not as stored rows). A database already on the current schema is untouched (every block is gated on the legacy column it targets still existing). Schema + one-time data migration, idempotent — verified locally with real drifted data (53 legacy holiday rows / 21 legacy attendance rows) then re-run twice with no further changes. | ✅ Yes (harmless no-op if the schema is already current) |
| `2026-08-21_patient_medical_conditions.sql` | Adds `patients.medical_conditions` (JSONB) — a fixed "Condition / History" checklist (Diabetes Mellitus, Hypertension, Thyroid Disorder, Asthma/COPD, Coronary Artery Disease, Tuberculosis, Epilepsy, Chronic Kidney Disease, Liver Disease, Cancer/Malignancy), each with Details + Currently in Treatment Yes/No, shown in the Prescription Builder below Prescription History. Patient-level (persists across visits), distinct from the existing free-text `patients.chronic_conditions` column; same array-on-patient storage pattern as `patients.symptoms`. Schema only. | ✅ Yes |
| `2026-08-24_optical_machine_prescribed_fields.sql` | Adds `right_machine_sph`/`right_machine_cyl`/`right_machine_axis`/`right_machine_add` and the matching `left_machine_*` columns to `optical_prescriptions` — the auto-refractometer/measurement-machine reading, stored separately from the existing `right_sph`/etc. columns (which stay the doctor's final prescribed values). Schema only. | ✅ Yes |
| `2026-08-24_lab_order_confirmatory_diagnosis.sql` | Adds `lab_orders.confirmatory_diagnosis` (TEXT) — the doctor's diagnosis confirmed once lab results come back, entered on the Lab Order Detail page and distinct from the provisional diagnosis recorded on the originating prescription. Schema only. | ✅ Yes |
