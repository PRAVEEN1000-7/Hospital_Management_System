...
const Layout: React.FC = () => {
  ...
  if (role === 'super_admin' || role === 'admin') {
    prescriptionItems.push(
      ...
      { to: '/prescriptions/new', label: 'New Prescription',  icon: 'add_circle' },
    );
  } else if (role === 'doctor') {
    prescriptionItems.push(
  ...
  const isActive = (path: string) => {
    ...
    if (path === '/prescriptions') {
      // Active for list page + detail/edit pages, but NOT for /prescriptions/new
      return p === '/prescriptions' ||
        (p.startsWith('/prescriptions/') && !p.startsWith('/prescriptions/new'));
    }
        ...
        <nav className="flex-1 py-4 overflow-y-auto custom-scrollbar">
          ...
          {prescriptionItems.length > 0 && (
            <div className="mt-4">
              ...
              >
                <div className="flex items-center">
                  <span className="material-symbols-outlined mr-3 text-[20px]">medication</span>
                  Prescriptions
                </div>
              ...
              <div className={`overflow-hidden transition-all duration-200 ${prescriptionsOpen ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}>
