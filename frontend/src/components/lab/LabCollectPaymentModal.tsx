...
interface LabCollectPaymentModalProps {
  orderId: string;
  onClose: () => void;
  /** Called once payment is successfully recorded, so the caller can refresh
   * its list (payment status/balance) in the background — fired immediately
   * on success, independent of whether the user has finished with/closed the
   * bill-view step that follows. */
  onPaid: () => void;
}

// Dialog version of the former full-page LabCollectPayment.tsx — collects a
// lab order's payment (with the same item-billing-amount editing, payment
// method, and Subtotal/Tax/Discount/Grand Total/Amount Received summary) and
// then shows the resulting bill for Print/Download, all without ever
// navigating away from the Lab Billing list. Layout condensed from the old
// full-page's checkout-style panels into a single scrollable dialog body.
const LabCollectPaymentModal: React.FC<LabCollectPaymentModalProps> = ({ orderId, onClose, onPaid }) => {
  ...
  // Report/Download PDF, a different document entirely). Replaces the
  // payment form within this same dialog rather than opening a second one.
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // closing the dialog; closing happens once the user is done with the bill.
  ...
  // The bill-view step takes over the whole dialog body (its own header with
  // Print/Download/Close) once payment succeeds — the payment form below it
  // is not rendered at the same time.
  if (paidInvoiceId) {
    return (
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 shrink-0">
          <h3 className="text-lg font-bold text-slate-900">Collect Lab Payment</h3>
            ...
            <>
              ...
              <div>
                ...
                {order.items.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-100 border border-slate-200 rounded-lg px-3">
...
export default LabCollectPaymentModal;
