...
const MyAppointments: React.FC = () => {
  ...
  // View appointment document modal — Print/Download live at the top-right
  // of this modal instead of a direct action, matching pharmacy/SalesList.tsx.
  const [viewingAppt, setViewingAppt] = useState<Appointment | null>(null);
  const [viewHtml, setViewHtml] = useState<string | null>(null);
  const [viewLoading, setViewLoading] = useState(false);
  const [downloadingAppt, setDownloadingAppt] = useState(false);
  ...
  const openViewAppt = async (appt: Appointment) => {
    setViewingAppt(appt);
    setViewHtml(null);
    setViewLoading(true);
    try {
      const html = await appointmentService.getAppointmentPdfUrl(appt.id);
      setViewHtml(html);
    } catch {
  ...
  const handleDownloadAppt = async () => {
    if (!viewingAppt || !viewHtml) return;
    setDownloadingAppt(true);
    try {
      await htmlStringToPdf(viewHtml, `Appointment_${viewingAppt.appointment_number}.pdf`);
    } catch {
      toast.error('Failed to download document');
    } finally {
      setDownloadingAppt(false);
    }
  };
  ...
  return (
    <div>
            ...
            <div key={appt.id} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-sm transition-shadow">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {/* View document - always available */}
                  <button onClick={() => openViewAppt(appt)}
                    className="p-1.5 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors" title="View">
      ...
      {/* View Appointment Document Modal — Print/Download at the top-right,
          matching pharmacy/SalesList.tsx's pattern. */}
      {viewingAppt && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closeViewAppt}>
          <div className="flex h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Appointment</h3>
                <p className="text-sm text-slate-500">{viewingAppt.appointment_number}</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={handlePrintAppt}
                  disabled={!viewHtml}
                ...
                <button
                  onClick={handleDownloadAppt}
                  disabled={!viewHtml || downloadingAppt}
                ...
                >
                  ...
                  {downloadingAppt ? 'Preparing…' : 'Download'}
                </button>
                <button onClick={closeViewAppt} className="ml-1 text-slate-400 hover:text-slate-600">
            ...
            <div className="min-h-0 flex-1 bg-slate-100">
              {viewLoading ? (
                ...
                <iframe
                  ref={viewIframeRef}
                  srcDoc={viewHtml}
                  title={`Appointment ${viewingAppt.appointment_number}`}
