...
const PRODUCT_CATEGORIES = [
  ...
  { value: 'optical', label: 'Optical', color: 'bg-purple-100 text-purple-800' },
  { value: 'surgical', label: 'Surgical', color: 'bg-red-100 text-red-800' },
  ...
  { value: 'disposable', label: 'Disposable', color: 'bg-yellow-100 text-yellow-800' },
...
const SupplierModal: React.FC<ModalProps> = ({ supplier, onClose, onSaved }) => {
  ...
  const toggleCategory = (category: string) => {
    setForm(p => ({
      ...p,
      product_categories: p.product_categories.includes(category)
        ? p.product_categories.filter(c => c !== category)
        : [...p.product_categories, category],
    }));
  };
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {PRODUCT_CATEGORIES.map(cat => {
                const isSelected = form.product_categories.includes(cat.value);
                return (
                  <button
                    key={cat.value}
                    type="button"
            ...
            <p className="text-xs text-slate-500 mt-1">Select all categories that apply</p>
          </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {s.product_categories && s.product_categories.length > 0 ? (
                          s.product_categories.slice(0, 3).map(cat => {
                            const category = PRODUCT_CATEGORIES.find(c => c.value === cat);
                            return (
                              <span
                                key={cat}
                                className={`px-2 py-0.5 rounded-full text-xs font-medium ${category?.color || 'bg-slate-100 text-slate-700'}`}
                              >
                                {category?.label || cat}
                              </span>
                            );
                        ...
                        {s.product_categories && s.product_categories.length > 3 && (
                          <span className="px-2 py-0.5 rounded-full text-xs bg-slate-100 text-slate-500">
                            +{s.product_categories.length - 3}
                          </span>
                        )}
                      </div>
                    </td>
